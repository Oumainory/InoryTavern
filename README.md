...
当项目有新版本发布时，你不需要手动执行复杂的命令。仓库根目录已经内置了**一键更新脚本**，包含拉取代码、安装依赖、生成 Prisma 客户端、同步数据库结构 4 步，无论未来更新了什么内容，使用此脚本均可保证环境一致性。

- **Windows 用户**：直接双击运行项目根目录下的 [`update.bat`](./update.bat) 文件即可自动完成全部更新流程。
- **Mac / Linux 用户**：在终端进入项目根目录，运行 `bash update.sh` 即可完成更新。

> 💡 **小贴士**：如果更新后前端行为异常，建议在更新前先停掉 `npm run dev` 进程（避免 Prisma 缓存旧 schema），更新完再重启即可。
